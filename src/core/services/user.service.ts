...
class CUserService {
  ...
  async registration({ email, password }: RegistrationCredentials) {
    ...
    if (condidate)
      throw ApiError.BadRequest({
        message: `Current email: [${email}] is already exist`,
      });
  ...
  async login({ email, password }: RegistrationCredentials) {
    ...
    if (!user) {
      throw ApiError.BadRequest({
        message: `Current email: [${email}] is not found`,
      });
    }
    ...
    if (!isPassIsEqual) {
      throw ApiError.BadRequest({
        message: `The specified password: [${password}] is incorrect`,
      });
    }
    const userDTO = new CUserDTO(user);
    const tokens = tokenService.generateToken({ ...userDTO });
  ...
  async setRole({ refreshToken, role }) {
    ...
    if (!refreshToken || !decodedToken) throw ApiError.UnauthorizedError();

    if (typeof decodedToken === 'string')
      throw ApiError.BadRequest({
        message: `Got value of type 'string', but need 'JwtPayload'`,
      });

    if (decodedToken.role === role)
      throw ApiError.BadRequest({
        message: `Сurrent role: [${role}] is already installed`,
      });

    if (role !== USER_ROLES.CREATOR && role !== USER_ROLES.REGULAR)
      throw ApiError.BadRequest({
        message: `Сurrent role: [${role}] does not exist`,
      });
